...
#[derive(Copy, Drop, Serde, Introspect, Display, Debug)]
struct Vec2 {
    x: u32,
    y: u32
}
...
impl PieceImpl of PieceTrait {
    ...
    fn is_right_piece_move(self: @Piece, next_position: Vec2) -> bool {
        ...
        match self.piece_type {
            ...
            PieceType::Bishop => {
                (n_x <= *self.position.x && n_y <= *self.position.y && *self.position.y- n_y == *self.position.x- n_x)   || 
                (n_x <= *self.position.x && n_y >= *self.position.y && n_y - *self.position.y == *self.position.x - n_x) ||
                (n_x >= *self.position.x && n_y <= *self.position.y && *self.position.y- n_y == n_x - *self.position.x)   ||
                (n_x >= *self.position.x && n_y >= *self.position.y && n_y -*self.position.y  == n_x - *self.position.x )
            },
            PieceType::Rook => {
...
//     #[test]
//     fn test_direction() {
//         let mut curr = Vec2 {x: 0, y: 0};
//         let mut next = Vec2 {x: 1, y: 1};
//         let (a, b, c, d): (u32, u32, u32, u32) = PieceTrait::get_direction(curr, next);
...
//     fn test_distance() {
//         let mut curr = Vec2 {x: 0, y: 0};
//         let mut next = Vec2 {x: 1, y: 2};
//         assert!(PieceTrait::get_distance(curr, next) == 2, "Wrong distance");
//     }
