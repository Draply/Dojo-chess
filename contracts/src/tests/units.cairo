...
mod tests {
    ...
    use chess::systems::actions::{actions, IActionsDispatcher, IActionsDispatcherTrait};
    ...
    fn test_move() { 
        ...
        let a3 = get!(world,(game_id,next_pos),(Piece));
        assert!(a3.piece_type == PieceType::None,"should be Pawn");
        let game_turn = get!(world, game_id, (GameTurn));
    ...
    fn test_win() {
        ...
        let curr_pos = Vec2 {x : 4,y : 1};
        let next_pos = Vec2 { x: 4, y: 2 };
        actions_system.move(curr_pos, next_pos, white.into(), game_id);
        // white end 

        let curr_pos = Vec2 {x: 3,y: 6};
        let next_pos = Vec2{x : 3, y:4 };
        actions_system.move(curr_pos, next_pos, black.into(), game_id);
        // black end

        let curr_pos = Vec2 {x: 5, y: 0};
        let next_pos = Vec2 {x: 1, y: 4};
        let a3 = get!(world,(game_id,curr_pos),(Piece));
        assert!(a3.piece_type == PieceType::Bishop,"Should be Bishop");
        actions_system.move(curr_pos, next_pos, white.into(), game_id);
        
        let curr_pos = Vec2 {x : 0, y : 6 };
        let next_pos = Vec2 {x: 0, y : 5 };
        actions_system.move(curr_pos, next_pos, black.into(), game_id);
        // black end
        let curr_pos = Vec2 {x : 1, y: 4};
        let next_pos = Vec2 {x: 4, y : 7};
        actions_system.move(curr_pos, next_pos, white.into(), game_id);

        let game_turn = get!(world, game_id, (GameTurn));
        assert!(game_turn.game_win == true,"Should be true");
    }
    #[test]
    #[should_panic]
    fn test_cant_move(){ 
